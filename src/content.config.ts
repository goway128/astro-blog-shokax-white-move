  ...
  schema: ({ image }) =>
    z
      .object({
        ...
        password: z.string().trim().min(1).optional(), // 构建时用于加密，不会输出到前端
      })
      .refine((data) => !data.encrypted || Boolean(data.password), {
        message: "Encrypted posts require a non-empty password",
        path: ["password"],
      }),
});
